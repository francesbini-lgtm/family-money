...
function getColValueStr(tx, colId, userAccounts) {
  ...
  switch(colId) {
    ...
    case 'compensatedAmt': return tx._compensatedAmt>0 ? fmtIT(tx._compensatedAmt,2) : '—'
    default:            return '—'
  }
}
  ...
  return (
    <tr className={'tx-row'+(tx.excluded?' excluded':'')}
      ...
      {cols.map(id => {
        ...
        if(id==='compensatedAmt') return (
          <td key={id} style={{padding:'4px 8px',textAlign:'right',fontSize:11,color:'var(--gold)',fontFamily:'var(--font-mono)'}}>
